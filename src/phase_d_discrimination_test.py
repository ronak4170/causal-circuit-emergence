"""
Follow-up to Phase D: does chain/fork/collider's decent tolerance-accuracy
(53-100% at various checkpoints) reflect genuine input-sensitive reasoning,
or could it be a memorized near-constant that happens to fall within
tolerance of the true answer?

Since association and intervention answers COINCIDE for these topologies by
Phase A's design (no confounding), tolerance-accuracy alone can't
distinguish "the model computed the right thing" from "the model outputs
something in the right ballpark regardless of the specific question."

Test: for each topology, build a larger set of questions (n=40) varying
do_value (True/False), which the true intervention answer is genuinely
sensitive to (forcing the treat_var True vs False really does change the
true probability, since these are real unconfounded causal edges). Check:
  1. Correlation between the model's predicted probability and the true
     answer across all questions (continuous fit quality -- near-zero would
     indicate the model isn't tracking the actual value at all).
  2. Whether the model's predictions correctly DISCRIMINATE do_value=True vs
     do_value=False -- i.e. does its mean prediction shift in the same
     direction as the true answers do between the two groups? A model
     outputting a near-constant regardless of do_value would fail this even
     if individual predictions happen to land within tolerance sometimes.
"""
import random
...
def main():
    print("Loading checkpoint 135 (final)...")
    model = load_model()
    ...
    pickle.dump({"raw_results": results, "summary": summary},
                open("results/phase_d_discrimination_results.pkl", "wb"))
    print("Saved results/phase_d_discrimination_results.pkl")
