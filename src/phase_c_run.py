"""
Step 3.3 + Step 5: run the full pre-registered Phase C experiment.
1. Compute mean-ablation targets for the candidate circuit.
2. Evaluate un-ablated and ablated on the interventional test set (confounded
   topology only, n=100 -- see PREREGISTRATION.md addendum).
3. Evaluate un-ablated and ablated on a count-matched associational test set.
Saves raw results for Step 4 (classification) and Step 5 (drop computation)
to consume separately.
"""
import pickle

from phase_b_setup import load_model
from phase_c_ablation import build_diverse_prompts, compute_component_means, build_ablation_hooks, evaluate
...
def main():
    ...
    pickle.dump(
        ...
        open("results/phase_c_raw_results.pkl", "wb"),
    )
    print("\nSaved results/phase_c_raw_results.pkl")
